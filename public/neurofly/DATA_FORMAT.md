...
The pure helper in `src/neurofly-training.js` converts a task and a validated local review record into a candidate observation–action pair. Inputs reference the uint8 demo crop, its shape/origin/calibration, the ordered trajectory history, the initial fragment graph, and candidate IDs, kinds and coordinates. All coordinates are crop-local XYZ voxels. No reference answer, held-out reviewer edge, or post-decision graph is included in the observation.
...
The helper documents the supervision format; it does not run a training or inference service. The architecture figure's task/candidate-conditioned structured action head is a proposed extension of NeuroFly's image/trajectory two-way attention model. The existing model predicts a 3D displacement from five trajectory positions and a rank/intensity image representation; the variable-length history and uint8 display crops here are not claimed to be its checkpoint-ready training tensors.

The structured-action illustration loads the real 96³ microscopy crop and uses exact saved coordinates and edges. `src/neurofly-rollout.js` splits the 55-node path into five initially visible fragments with inclusive index ranges `[0,4]`, `[11,16]`, `[24,28]`, `[37,42]`, and `[49,52]`. The deterministic sequence contains 29 one-point extensions and four joins. Joins 1 and 3 select **Accept** in the fragment-connection head; joins 2 and 4 select endpoint **B** over endpoint **C** (the opposite end of the same nearby fragment). Both candidates lie inside a 32³ window centered on the source. Each join adds the missing adjacent source edge and incorporates the existing fragment, rather than revealing its nodes one at a time. Extension choices use the next recorded point as candidate B; they illustrate an action rather than running a proposal model. No artificial point or edge is committed to the graph. The final crop endpoint is not treated as a true biological ending and does not emit **None**.

`data/tracing-context.json` packages all source annotations intersecting the same physical crop: 155 interior nodes, 10 outside endpoints, and 160 edges in 16.7 KB. Segment–box intersection uses voxel bounds `[-0.5, shape−0.5]`; exported coordinates are unmodified source positions minus the crop origin. The browser clips these lines to the displayed volume and retains them in a static context layer. It excludes only the active path's original edges from that layer, so those edges can be revealed by the action sequence. The context and sequence together show the annotated graph throughout the volume. Rebuild this context with `python scripts/export_neurofly_tracing_context.py --source /path/to/RM009_axons_2.db`; the standard-library exporter verifies the published geometry fingerprint and opens the source read-only.

Each state contains the visible node indices and index-pair edges, connected prefix, current tip, most recent edge, candidate labels/indices, task type, chosen option, and remaining fragment count. It is independent of playback history, so scrubbing backward restores disconnected fragments and synchronizes the precise model-head option. Connected geometry is cyan and unconnected fragments are lavender; the current action and its candidate edge are highlighted. Orbit/zoom, play/pause, restart, and the step slider remain available. Playback pauses during camera interaction, offscreen, and in a hidden tab; reduced-motion preferences disable automatic playback. The model diagram retains the real 32³ oblique MIP and five source history points.

`data/tracing.json` records the exact 55-node path from source node 1242 to 1188, with 54 existing edges (53 `seger`, one saved `tester` join between 1194 and 1193). Its arc length is 202.4384 µm. The 96³ crop starts at source XYZ `[107,347,52]`, with 1 µm spacing, and is stored in `tracing-volume.u8.gz` (107,054 bytes). Path coordinates are original database positions minus the crop origin; there is no resampling, interpolation, or smoothing of the fiber. The metadata retains source geometry fingerprints, edge provenance, display quantization, and asset checksums.

The model figure uses a real **32³ subcrop** at parent-volume origin `[12,30,48]`, absolute source origin `[119,377,100]`. Its five trajectory points are original nodes 1222–1218 in this local coordinate system. `tracing-context-oblique.png` is a true volume MIP, raycast with 64 trilinear samples per valid ray through this subcrop. The SVG's trajectory and image share the exact projection `screen = [250,560] + qx*[315,-26] + qy*[0,236] + qz*[-155,-54]`, where `q = (p + 0.5) / 32`; image bounds are `[95,480,470,316]`. The raycast image is transparent outside the projected cube. The additional `tracing-context-xy/xz/yz.png` files are exact orthogonal MIPs without further normalization; `tracing-xy/xz/yz.png` document the complete replay crop.

Reproduce the replay assets with `python scripts/export_neurofly_tracing.py --source /path/to/labeled_blocks`. The exporter opens the image and database read-only, checks the published source geometry fingerprint, validates every consecutive graph edge and coordinate, and verifies the saved volume and projections against the source.
