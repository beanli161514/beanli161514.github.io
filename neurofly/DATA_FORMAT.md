...
The fiber replay loads a real 96³ microscopy crop and reveals successive saved annotation nodes and their existing connecting edges in a shared 3D coordinate system. It does not generate new geometry or predict nodes. The view supports orbit/zoom, play/pause, restart, and a step slider. Playback pauses during camera interaction, offscreen, and in a hidden tab; reduced-motion preferences disable automatic playback. Geometry is uploaded once and the visible point/edge counts change with the timeline. The model diagram uses real maximum-intensity projections from the same crop on its illustrative volume cube.
